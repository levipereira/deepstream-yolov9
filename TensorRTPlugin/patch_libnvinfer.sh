...
if [ ! -f "./libnvinfer_plugin.so.8.6.1.$arch" ]; then
    echo -e "\n\nDownloading: libnvinfer_plugin.so.8.6.1"
    curl -L --retry 3 --retry-delay 5 -A "Mozilla/5.0" -o "./libnvinfer_plugin.so.8.6.1.$arch" "https://github.com/levipereira/deepstream-yolov9/releases/download/v1.0/libnvinfer_plugin.so.8.6.1.$arch" || exit 1
fi
...
cp $(pwd)/libnvinfer_plugin.so.8.6.1.$arch $TRT_INSTALL_LIBPATH/libnvinfer_plugin.so.8.6.1
